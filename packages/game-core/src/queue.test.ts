...
describe('ordered offline check-ins', () => {
  it('holds later check-ins behind a network failure and syncs them in order after recovery', async () => {
    const checkIn = vi.fn().mockRejectedValueOnce(ApiError.network(null)).mockResolvedValue({ checkInId: 'c', baseId: 'b', checkedInAt: 'now' })
    const { queue, tick } = make({ checkIn })
    await queue.enqueueCheckIn({ id: 'q1', gameId: 'g', baseId: 'b1', nfcToken: 't' })
    await queue.enqueueCheckIn({ id: 'q2', gameId: 'g', baseId: 'b2', nfcToken: 't', prerequisiteCheckInIds: ['q1'] })
    await queue.sync()
    expect(checkIn).toHaveBeenCalledTimes(1)
    tick(2001)
    await queue.sync()
  ...
  it('permanently rejects dependent proofs after an earlier tag fails, requiring a fresh scan', async () => {
    const checkIn = vi.fn().mockRejectedValue(ApiError.fromResponse(403, { code: 'INVALID_TOKEN' }))
    const { queue } = make({ checkIn })
    await queue.enqueueCheckIn({ id: 'q1', gameId: 'g', baseId: 'b1', nfcToken: 't' })
    await queue.enqueueCheckIn({ id: 'q2', gameId: 'g', baseId: 'b2', nfcToken: 't', prerequisiteCheckInIds: ['q1'] })
    await queue.sync()
  ...
  it('preserves the missing base number and does not retry refused out-of-order proofs', async () => {
    ...
    await queue.enqueueCheckIn({ id: 'q3', gameId: 'g', baseId: 'b3', nfcToken: 't' })
...
it('a fresh scan replaces the refused proof so it cannot block later submissions', async () => {
  const checkIn = vi.fn().mockRejectedValueOnce(ApiError.fromResponse(400, { code: 'PREVIOUS_BASE_REQUIRED', errors: { nextRequiredBaseNumber: '1' } })).mockResolvedValue({ checkInId: 'c', baseId: 'b2', checkedInAt: 'now' })
  const { queue, exec } = make({ checkIn })
  await queue.enqueueCheckIn({ id: 'old', gameId: 'g', baseId: 'b2', nfcToken: 'old' })
  await queue.sync()
  await queue.enqueueCheckIn({ id: 'fresh', gameId: 'g', baseId: 'b2', nfcToken: 'new' })
