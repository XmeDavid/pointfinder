...
export function TeamDetailPage() {
  ...
  const statusIcon: Record<SubmissionStatus, React.ReactNode> = { pending: <Clock className="h-4 w-4 text-yellow-500" />, approved: <CheckCircle className="h-4 w-4 text-green-500" />, rejected: <XCircle className="h-4 w-4 text-red-500" />, correct: <CheckCircle className="h-4 w-4 text-green-500" />, incorrect: <XCircle className="h-4 w-4 text-red-500" /> };
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          ...
          <CardContent>{players.length === 0 ? <p className="text-sm text-muted-foreground">{t("teams.noMembers")}</p> : <div className="space-y-2">{players.map((p) => (<div key={p.id} className="flex items-center justify-between text-sm"><span className="font-medium">{p.displayName}</span><span className="text-xs text-muted-foreground font-mono">{p.deviceId}</span></div>))}</div>}</CardContent>
        </Card>
        <Card>
