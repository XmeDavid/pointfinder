...
M={k:material(k,v) for k,v in {
    ...
    'Scarf gold':'F2C568','Scarf red':'B84042','Warm skin':'C99061',
    'Ear warmth':'B57951','Hair chestnut':'493025','Hair highlight':'58382B',
...
def scarf(guide):
    panel('Neckerchief / back triangle',[(-.25,.175,1.755),(0,.255,1.465),(.25,.175,1.755)],'Scarf navy',group='scarf',thick=.018)
    tube('Neckerchief / gold border',[(-.25,.187,1.75),(0,.269,1.475),(.25,.187,1.75)],.012,'Scarf gold',group='scarf',smooth=False)
    if guide:tube('Neckerchief / red edge',[(-.257,.191,1.76),(0,.279,1.454),(.257,.191,1.76)],.007,'Scarf red',group='scarf',smooth=False)
    for side in [-1,1]:
        # Rolled scarf from nape to the woggle, with diagonal woven bands.
        path=[Vector((side*.13,.10,1.79)),Vector((side*.20,-.02,1.77)),Vector((side*.21,-.17,1.71)),Vector((side*.14,-.241,1.64)),Vector((0,-.267,1.59))]
        pts=[]
...
def boots(side,suffix):
    ...
    box('Boot / ankle '+suffix,(x,.015,.24),(.233,.245,.17),'Leather',.045,b,'boots')
...
def body(style,guide):
    ...
    for side in [-1,1]:
        ...
        sphere('Eye',(side*.112,-.313,2.10),(.015,.014,.045),'Eyes','head','face',20,12)
    hair(style)
    ...
    for o in parts:
        ...
        if 'guide' in rig.name and o['part_group']=='hair':
            ...
            for v in o.data.vertices:
                v.co.x*=.94;v.co.y*=.94;v.co.z=2.08+(v.co.z-2.08)*.54
        if 'guide' in rig.name and o.name.startswith('Face / rounded silhouette'):
            # Match the fitted scalp to the shorter hairstyle so removing the
            # hat reveals hair, rather than the head protruding through it.
            for v in o.data.vertices:
                if v.co.z>2.19:v.co.z=2.19+(v.co.z-2.19)*.35
        bone=o['bind_bone']
...
for key,style,guide in [('pathfinder-short','short',False),('pathfinder-ponytail','ponytail',False),('pathfinder-guide','short',True)]:
    ...
    body(style,guide);rig=skeleton(key);bind(rig);animations(rig)
...
area('Key / large softbox',(-3,-4,7),650,(1,.86,.70),5)
area('Fill / cool card',(4,-2,4),400,(.76,.87,1),4)
area('Rim / warm edge',(1,4,6),750,(1,.88,.73),3.5)
...
stage_all();camera((0,-10,4.3),(0,0,1.3),5.5)
...
# Pose sheet: one model, six reusable examples, drawn from the same actual rig.
for idx,(name,angle) in enumerate([('idle',0),('wave',-.12),('point',-.2),('phone',-.1),('celebrate',0),('walk',-.25)]):
    for i,(_,rig,objs,col) in enumerate(characters):col.hide_render=i!=0
    rig=characters[0][1];rig.location=(0,0,0);rig.rotation_euler=(0,0,angle);pose(rig,name,.25)
    camera((3,-7,3.2),(0,0,1.35),3.2)
    S.render.resolution_x=S.render.resolution_y=800
    S.render.filepath=str(OUT/f'pose-{name}.png');bpy.ops.render.render(write_still=True)

# Back detail / accessories.
for i,(_,rig,objs,col) in enumerate(characters):col.hide_render=i!=2
rig=characters[2][1];rig.location=(0,0,0);rig.rotation_euler=(0,0,0);reset(rig)
camera((-3,7,3.6),(0,0,1.36),3.2)
S.render.filepath=str(OUT/'guide-back.png');bpy.ops.render.render(write_still=True)

# Export from a duplicate working state, leaving the authored blend intact.
report={}
for key,rig,objs,col in characters:
    col.hide_render=False;rig.location=(0,0,0);rig.rotation_euler=(0,0,0);reset(rig)
    # Collapse by accessory group; retain the shared skin and editable module names.
    groups={}
    for o in objs:groups.setdefault(o['part_group'],[]).append(o)
    joined=[]
    for group,items in groups.items():
        bpy.ops.object.select_all(action='DESELECT')
        for o in items:o.select_set(True)
        bpy.context.view_layer.objects.active=items[0];bpy.ops.object.join()
        o=bpy.context.object;o.name=key+'/'+group;joined.append(o)
        matrix=o.matrix_world.copy();o.parent=None;o.matrix_world=matrix
        for layer in list(o.data.uv_layers):
            if group!='scarf' or layer.name!='Scarf UV':o.data.uv_layers.remove(layer)
        if group=='scarf':
    ...
    bpy.ops.export_scene.gltf(filepath=str(OUT/(key+'.glb')),export_format='GLB',use_selection=True,
        ...
        export_skins=True,export_yup=True,export_apply=False,export_materials='EXPORT')
    report[key]={'bones':len(rig.data.bones),'modules':list(groups),'file':key+'.glb','bytes':(OUT/(key+'.glb')).stat().st_size,
        'vertices':sum(len(o.data.vertices) for o in joined),'poses':['idle','wave','point','celebrate','phone','walk']}
(OUT/'asset-manifest.json').write_text(json.dumps(report,indent=2)+'\n')
(OUT/'blender_assets.cats.txt').write_text('VERSION 1\n\nbfbe4b59-00dd-430c-82b6-eaf0d071c433:PointFinder/Characters:Characters\n')
print('CHARACTER_LIBRARY_COMPLETE',flush=True)
