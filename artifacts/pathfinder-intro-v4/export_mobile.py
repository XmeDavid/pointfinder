...
(OUT/'timeline.json').write_text(json.dumps({'fps':24,'frameStart':1,'frameEnd':864,'holds':[125,301,371,465,580,765,864],'worldFade':[782,814],'camera':camera_frames},separators=(',',':')))
...
export(world,'world.glb',True)
export(compass,'compass.glb',False)
stats={'sourceObjects':len(scene.objects),'worldMeshGroups':world_groups,'worldBytes':(OUT/'world.glb').stat().st_size,'compassBytes':(OUT/'compass.glb').stat().st_size,'compassNeedle':'PF_CompassNeedle','compassNeedleAxis':'y','compassNorthLocal':[0,0,-1],'cameraCoordinateSystem':'glTF Y up','terrainTextures':len(terrain_materials),'characterLibrary':'pathfinder-characters-v1','characterRigs':len(character_rigs),'characterBones':[len(r.data.bones) for r in character_rigs],'characterVertices':character_vertices}
