...
def optimize(root):
    sources=[o for o in root.children_recursive if o.type in {'MESH','CURVE'}]
    groups={}
    for obj in sources:
...
stats={'sourceObjects':1011,'worldMeshGroups':world_groups,'worldBytes':(OUT/'world.glb').stat().st_size,'compassBytes':(OUT/'compass.glb').stat().st_size,'compassNeedle':'PF_CompassNeedle','compassNeedleAxis':'y','compassNorthLocal':[0,0,-1],'cameraCoordinateSystem':'glTF Y up','terrainTextures':len(terrain_materials)}
