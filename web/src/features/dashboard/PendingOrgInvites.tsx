...
import { useAcceptOrgInvite, useDeclineOrgInvite } from '@/hooks/mutations/useOrgMutations'
  ...
  return (
    <div className="mb-6 space-y-2">
      {invites.map((invite) => (
        ...
        >
          ...
          <button
            ...
            disabled={busy}
            data-testid="org-invite-decline"
            className="shrink-0 px-3 py-1.5 rounded-lg text-xs font-medium text-muted-foreground hover:text-foreground transition-colors disabled:opacity-50"
          >
            {t('org.declineInvite', 'Decline')}
          </button>
          <button
      ...
      {declineInvite.isError && (
        <p className="text-sm text-destructive" role="alert" data-testid="org-invite-error">
