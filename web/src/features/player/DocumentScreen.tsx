...
export default function DocumentScreen() {
  ...
  const resource = resources?.find((r) => r.id === resourceId && r.type === 'document')
    ...
    <Screen>
      ...
      {resource && (
        <article data-testid="document-body">
          ...
          <RichContent html={resource.content ?? ''} className="prose prose-sm mt-4 max-w-none text-foreground dark:prose-invert" />
        </article>
      )}
    </Screen>
  )
