...
export function NfcTagsManager({
  ...
  const [printBaseId, setPrintBaseId] = useState<string | null>(null)

  const defaultRadius = game.data?.defaultCheckInRadiusM

  const visible = useMemo(() => {
    const list = [...(bases.data ?? [])].sort(
      (a, b) =>
        Number(a.nfcLinked) - Number(b.nfcLinked) ||
        a.name.localeCompare(b.name),
    )
    const byMethod =
      methodFilter === 'all'
        ? list
        : list.filter((b) => b.checkInMethod === methodFilter)
    ...
    return byMethod.filter(
      (b) =>
        b.checkInMethod !== 'NFC' ||
        (filter === 'linked' ? b.nfcLinked : !b.nfcLinked),
    )
    ...
    () =>
      (bases.data ?? [])
        ...
        .map((b) => ({
          id: b.id,
          name: b.name,
          value: printableTagUrl(b.id, b.nfcToken),
        })),
    [bases.data],
  )
  ...
  return (
    <div
      className={`${standalone ? 'mx-auto max-w-2xl' : 'w-full'} flex h-full flex-col gap-4 overflow-y-auto px-4 py-4`}
      data-testid="nfc-tags-page"
    >
      {standalone && (
        <Link
          to={`/game/${encodeURIComponent(gameId)}`}
          className="inline-flex items-center gap-1 text-sm text-muted-foreground"
        >
          <ChevronLeft className="h-4 w-4" aria-hidden />{' '}
          {game.data?.name ?? tCommon('common.back')}
        </Link>
      )}
      <header className="flex flex-col gap-1">
        <h1 className="text-2xl font-semibold leading-tight">
          {tCommon('checkIn.tagsAndCodes')}
        </h1>
      ...
      <div
        ...
        role="tablist"
        aria-label={tCommon('checkIn.method')}
        data-testid="codes-method-filter"
      >
        {methodFilters.map((f) => (
          <Button
            key={f.key}
      ...
      {bases.data && visible.length === 0 && (
        <EmptyState
          icon={<Nfc className="h-6 w-6" aria-hidden />}
          title={
            qrCodes.length === 0 && methodFilter === 'QR'
              ? tCommon('checkIn.noQrBases')
              : t('noBases')
          }
        />
      )}
                ...
                <div className="min-w-0">
                  ...
                  {base.hidden && (
                    <p className="text-xs text-muted-foreground">
                      {tCommon('bases.hidden', { defaultValue: 'Hidden' })}
                    </p>
                  )}
                </div>
              ...
              {base.checkInMethod === 'QR' && (
                <div className="flex items-center gap-3">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      setPrintBaseId(base.id)
                      setPrintOpen(true)
                    }}
                  >
                    {tCommon('checkIn.printCode')}
                  </Button>
                </div>
              )}
              {base.checkInMethod === 'LOCATION' && (
                <p className="text-xs text-muted-foreground">
                  {tCommon('checkIn.noTagNeeded')} {tCommon('checkIn.radius')}:{' '}
                  {resolveCheckInRadiusM(base.checkInRadiusM, defaultRadius)} m
                </p>
              )}
            </li>
          ))}
      ...
      <CodesPrintSheet
        ...
        codes={
          printBaseId
            ? qrCodes.filter((code) => code.id === printBaseId)
            : qrCodes
        }
