...
Any signed-in account may report a listed game (V86), one open report per account and game; reports on unlisted games are 404 like every other Explore read. Only platform admins read reports, including the reporter's name, and resolve all of a game's open reports at once: dismiss keeps the listing, remove is the ordinary unpublish audited as the admin's action. Publishers never see reports or reporters. Any admin removal holds the listing (V87): the publisher keeps editing the draft but gets `PUBLICATION_ON_HOLD` when listing it until an admin releases it; hold and release are publication events. The first open report on a listing emails `app.moderation.alert-emails` after commit; further reports while it is open do not. An admin can block any non-admin account with a reason: the token version is bumped, refresh tokens are removed, sign-in, refresh and organizer-session exchange are refused with `ACCOUNT_BLOCKED` (checked only after a correct password), and its listings are removed and held. Unblocking restores sign-in only. Player tokens of a blocked participant account are separate and are not revoked.
...
Realtime stats describe only the answering instance (`scope: "instance"`, `instanceId`). Dead-lettered outbox rows are inspected by platform admins, never replayed: every event is a refresh signal and clients converge on a snapshot.

An upload is stalled when it is active and unexpired but no chunk arrived for `app.uploads.stalled-threshold-minutes`, and unlinked when it completed without a submission for `app.uploads.needs-attention-threshold-minutes`. Both are read-only operator signals; nothing is expired, deleted or relinked because the player's device still holds the work.
