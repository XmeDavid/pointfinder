...
| Illustrated welcome stories: role choice, organizer account gate and signed-in tour offer, participant and organizer chapters, landing per audience (visitor: native Join / browser iOS and Android downloads, operator with first-game hand-off, player), three languages, reduced motion and image fallback | yes (opt-in `?onboarding=choice`, `?onboarding=gate`, `?onboarding=1`–`5`, `&role=organizer`, `&mode=operator|player`) | n/a | n/a |
