...
Goal: ensure `info@pointfinder.pt` forwards reliably to the maintained personal inbox.

## Inputs

- Domain registrar / DNS provider access for `pointfinder.pt`
...
3. Keep app defaults aligned with store contact:
   - `docker-compose.yml` uses `MAIL_FROM=info@pointfinder.pt`
   - `backend/src/main/resources/application.yml` defaults to `MAIL_FROM=info@pointfinder.pt`

## Verification checklist

- [ ] Send test email from an external mailbox (Gmail/Outlook) to `info@pointfinder.pt`
...
2. Keep `info@pointfinder.pt` published on the privacy page only after routing is restored.
