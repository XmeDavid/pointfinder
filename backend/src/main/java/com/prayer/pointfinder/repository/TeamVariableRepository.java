...
public interface TeamVariableRepository extends JpaRepository<TeamVariable, UUID> {
    ...
    @Query("SELECT tv.variableKey, COUNT(DISTINCT tv.team.id) FROM TeamVariable tv " +
           "WHERE tv.game.id = :gameId GROUP BY tv.variableKey")
    List<Object[]> countTeamsPerKeyByGameId(@Param("gameId") UUID gameId);
}
